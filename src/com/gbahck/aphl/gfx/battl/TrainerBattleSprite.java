 ...
 *
 */
public class TrainerBattleSprite extends BattleSprite {

	public TrainerBattleSprite( MemoryBuffer memory, int id ) {
		super(getPixels( memory, id ), getPalette( memory, id ), 64);
	}
	
	private static BitmapPixelData getPixels( MemoryBuffer memory, int id ) {
		int address = memory.getPointer( 0x23957C + ( id * 8 ) );
		return new BitmapPixelData( memory, address, BitmapPixelDepth.BPP_4, CompressionType.LZ77 );
	}
	
	private static BitmapPaletteData getPalette( MemoryBuffer memory, int id ) {
		int address = memory.getPointer( 0x239A1C + ( id * 8 ) );
